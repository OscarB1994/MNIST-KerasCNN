...
if True:
    ...
    epochs      = 8   # how many times to loop over the entire training dataset
    ...
    model.add(MaxPooling2D((2, 2),padding='same'))
    model.add(Conv2D(256, kernel_size=(5, 5),activation='relu',input_shape=(28,28,1),padding='same'))
    model.add(MaxPooling2D((2, 2),padding='same'))
...
confusion = confusion_matrix(test_Y_one_hot.argmax(axis = 1), model.predict(test_X).argmax(axis = 1))
ax = sns.heatmap(confusion, annot=True, fmt="d", cmap="Greens")
